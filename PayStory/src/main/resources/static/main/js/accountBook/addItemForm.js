...
$(function() {
	...
	$('#incomeForm').on('submit', function(e) {
		...
		let address = "/accountBook/income";

		// 내역 수정 시
		if ($('#isEdit').val()) {
			let condition = $('#condition').val();
			let dataNo = $('#hiddenIncomeData').children('input').eq(5).val();

			address = "/income/" + condition + "/" + dataNo;
		}
	...
	$('#expenditureForm').on('submit', function(e) {
		...
		let address = "/accountBook/expenditure";
